...
#                 scores1 = self.model(data)
#                 log_probs = nn.LogSoftmax(dim=1)(scores1)
...
#         # Step1Cand = scores[:top_N].cpu().numpy()
#         DiverCandScore = []
#         # DiverCandScore = DiverCandScore.to(self.device)

#         # Fist Criterion: Uncertainty based on data augmentation's consistency
#         FinalCombine = []
#         scores = scores.cpu().numpy()
#         DivScore = DivScore.cpu().numpy()
#         Entropy = Entropy.cpu().numpy()
#         for i in range(0, top_N):
#             indexCand = scores[i]


#             # a = torch.Tensor(DivScore[indexCand]*UncCand)
#             # a = a.to(self.device)
#             # print(type(DivScore[indexCand].item()))
#             # print(type(UncCand.item()))
#             a = DivScore[indexCand].item()*(Entropy[indexCand].item())

#             TmpCombine = np.array((indexCand, a))
#             FinalCombine.append(TmpCombine)

#         FinalCombine = np.array(FinalCombine)
#         FinalCombine = FinalCombine[np.lexsort(FinalCombine.T)]
#         finalindex = FinalCombine[:, 0]
#         top_N = int(len(finalindex)*0.2)
#         q_idxs = np.random.choice(finalindex[top_N:], n, replace=False)
#         # print(q_idxs)
#         # 上面一步只是确实距离 后面需要乘熵 上面关键还在于i和sample_weights中的index
#         q_idxs = q_idxs.astype('int64')
#         return idxs_unlabeled[q_idxs]
