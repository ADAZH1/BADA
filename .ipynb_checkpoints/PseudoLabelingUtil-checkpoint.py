...
def pseudo_labeling(args,  device, data_loader, model, itr, unc, log_prob, num_classes, PLacc):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (( inputs, data_t_raug), targets, indexs, _) in enumerate(data_loader):
            ...
            interm_nl_mask = ((out_std_nl < 0.005 ) * (out_prob_nl < 0.2)) * 1    # Core Code  选择负学习的相关标签 我这边是只根据
            ...
            selected_idx = (max_value >= 0.8) * (max_std.squeeze(1) < 0.3)
