...
def train_regular(args, device, num_classes,lbl_loader, nl_loader, model, optimizer, epoch, itr, unlbl_loader, src_train_loader):
    ...
    train_loader = zip(lbl_loader, nl_loader, unlbl_loader, src_train_loader)  # 这个整个还是unlbl_dataloader

    model.train()
    # for batch_idx, ((inputs, data_t_raug), targets, indexs, _) in enumerate(data_loader):
    for batch_idx, (data_x, data_nl, data_unlbl, data_src) in enumerate(train_loader):
        data_time.update(time.time() - end)
        (inputs_x, _) ,targets_x, _, nl_mask_x = data_x
        (inputs_nl,_), targets_nl, _, nl_mask_nl = data_nl
        (inputs_unlbl, data_t_raug), targets_unlbl, indexs, _ = data_unlbl
        ...
        inputs_unlbl = inputs_unlbl.to(device)
        score_tu = model(inputs_unlbl)
        for data_aug_curr in data_t_raug:
            score_t_aug_curr = model(data_aug_curr.to(device))
        ...
        # loss = loss_ce # validate supervised
        loss.backward()
