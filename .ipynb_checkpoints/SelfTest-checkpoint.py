...
def run_active_adaptation(args, source_model, src_dset, num_classes, device):
    ...
    PLacc = defaultdict(list)
    # print(log_prob.shape)   # log_prob存放着 之前计算每个样本的方差?
    PLacc_name = '{}_positive labels acc'.format(args.id)
...
def main():
    ...
    parser.add_argument('--num_rounds', type=int, default=1, help="Target dataset number of splits")  # 1 num_rouds
