...
def run_active_adaptation(args, source_model, src_dset, num_classes, device):
    ...
    print(budget_all)
    args.total_budget = budget_all * 0.01
    ...
    # Setup
    #################################################################
    Postive_acc = defaultdict(list)  # 目前尚未采取相应的DA Strategy
            ...
            for itr in range(start_itr, 3):
                ...
                for epoch in range(0, 60):
                    ...
                    # else:
                        ...
                        # train_loss = train_regular(args, device, num_classes, lbl_loader, nl_loader, best_model,optimizer, epoch, itr, unlbl_loader, src_train_loader)
                ...
                print('\n------------------------------------------------------\n')
                print(out_str)
                target_accs[ratio+itr].append(test_perf)
            print("正负学习之后的准确率")
            test_perf, _ = utils.test(best_model, device, target_test_loader, "test", num_classes)
            train_perf, _ = utils.test(best_model, device, target_train_loader, "test", num_classes)
...
def main():
    ...
    parser.add_argument('--total_budget', type=int, default=44, help="Total target budget") # 只在一轮sample selection之后做自训练的实验
