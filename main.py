...
def run_active_adaptation(args, source_model, src_dset, num_classes, device):
    ...
    if args.da_strat != 'mme':  # MME
        ...
        model, src_model, discriminator = utils.run_unsupervised_da(source_model, src_train_loader, None, target_train_loader, \
    ...
    for run in tqdm_run:
        ...
        sampling_strategy = get_strategy(args.al_strat, src_train_loader, target_train_dset, target_train_loader, target_train_idx,  \
                                         curr_model, discriminator, device, src_seq_loader, args)  # default: Clue-main  主动学习策略的实例化
        ...
        unc, log_prob = sampling_strategy.PseudoBasedRank(fina0model, target_train_loader)
        # print(unc.shape)
        ...
        # Evaluate on target test and train splits
        print("正负学习之前的准确率")
        test_perf, _ = utils.test(fina0model, device, target_test_loader, "test", num_classes)
        train_perf, _ = utils.test(fina0model, device, target_train_loader, "test", num_classes)
        ...
        for itr in range(start_itr, 2):
            ...
            for epoch in range(0, 50):
                if itr == 0:
                    # print("itr==0")
                    train_loss = train_initial(args, device, num_classes, lbl_loader, fina0model, optimizer, epoch,
                    ...
                    # train_loss = train_regular(args, device, num_classes, lbl_loader, nl_loader, fina0model,
                    #                                optimizer, epoch, itr)
                    train_loss = train_regular(args, device, num_classes, lbl_loader, nl_loader, fina0model,
            ...
            # 需要一次机会生成伪标签
            pseudo_lbl_dict = pseudo_labeling(args, device, unlbl_loader, fina0model, itr, unc, log_prob,
                                                  num_classes)
            print("伪标签生成结束")
            lbl_dataset, nl_dataset, unlbl_dataset = PLNLUtils.get_dataset(args, args.target,
                                                                               train_lbl_idx,
...
def main():
    ...
    parser.add_argument('--cfg_file', type=str, help="Experiment configuration file", default="config/OfficeHome/Consistencyderived.yml")
    ...
    source_model = get_model(args.cnn, num_cls=num_classes, l2_normalize=args.l2_normalize, temperature=args.temperature).to(device)
